...
const PasswordReset = () => {
    ...
    const onSubmit = async (data:ResetPasswordSchemaType) =>{
        try{
            ...
            console.log({...data, id: params?.get('id'), email: params?.get('email')});
            
            const response = await axios_instance.post("/auth/clients/reset-password", {
                ...data, id: params?.get('id'), email: params?.get('email')
            })
        ...
        <>
            <header className='w-full py-4 border-b absolute z-50'>
                ...
                <Link to={"../"} className='flex gap-2 items-center'>
                    ...
                    <h1 className='text-3xl font-bold text-black'>CSL Freight</h1>
                </Link>
                </nav>
            </header>
                ...
                <div className='h-screen max-h-[800px] flex items-center justify-center sm:flex-col lg:flex-row'>
                    <Form {...form}>
                        <form className='p-4 lg:p-8 rounded-lg border' onSubmit={form.handleSubmit(onSubmit)}>
                            ...
                            <p className='text-xs mb-3 lg:mb-6 2xl:mb-8 text-gray-400' >Time for a fresh start! Go ahead and set a new passord for {params?.get('email')}.</p>
