...
const ShopProducts = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let patchId;
      if (editingProduct) {
        patchId = editingProduct.productId ?? editingProduct.id;
      ...
      // Always upload images if present, and only proceed if upload succeeds
      if (patchId && form.imageFiles.length > 0) {
        const uploadSuccess = await uploadImages(patchId, form.imageFiles);
        if (!uploadSuccess) {
          setSubmitting(false);
          return; // Don't close modal or refresh if upload failed
        }
      }
      await fetchProducts();
      closeModal();
