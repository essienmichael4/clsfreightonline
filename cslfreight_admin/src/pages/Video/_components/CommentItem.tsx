...
const CommentItem = ({ comment, videoId, clientId, userId }: Props) => {
  ...
  const isOwner =
    userId === comment.user?.id || clientId === comment.client?.id || isAdmin;
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      await axios_instance_token.delete(`/videos/${videoId}/comments/${comment.id}`, {
        data: { author: {  userId } },
      });
    },
    onSuccess: () => {
  ...
  const editMutation = useMutation({
    mutationFn: async () => {
      ...
      const res = await axios_instance_token.patch(
        `/videos/${videoId}/comments/${comment.id}/admin`,
        { content: editText }
      );
      return res.data;
    },
