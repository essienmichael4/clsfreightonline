...
import { Package, PackageTypeAndRate, Stats } from '@/types';
import { startOfMonth, subMonths } from 'date-fns';
import { Rate } from 'app/_lib/types';
...
export default function DashboardScreen() {
    ...
    if (rates.isLoading || stats.isLoading || orders.isLoading) {
        return (
    ...
    if (rates.isError || stats.isError || orders.isError) {
        return (
            <SafeAreaView style={styles.container}>
                <LinearGradient
                    colors={[colors.white, colors.darkGray]}
            ...
            >
                ...
                >
                    ...
                    <View style={styles.shippingRateCard}>
                        ...
                        <Text style={styles.exchangeRateLabel}>USD → GHS Rate</Text>
                                ...
                                {packageData.length === 0 ? (
                                    ...
                                    currentPageData.map((pkg, index) => (
                                        <View key={index} style={styles.tableRow}>
                                            ...
                                            <Text style={[styles.tableCell, styles.cellDate]}>{pkg.received?.split('T')[0]}</Text>
                                            <Text style={[styles.tableCell, styles.cellDate]}>{pkg.loaded?.split('T')[0]}</Text>
                                            <Text style={[styles.tableCell, styles.cellDate]}>{pkg.eta?.split('T')[0] === pkg.loaded?.split('T')[0] ? '-' : pkg.eta?.split('T')[0]}</Text>
